...
public class Metadata {
    ...
    static final int METADATA_SIZE = (Integer.BYTES * 3) + (Byte.BYTES * 3);
    ...
    private Metadata(int segmentSize, int segmentBitShift, int maxSegmentsPerLevel, boolean mmap, boolean flushAfterWrite, boolean asyncFlush) {
    ...
    public static Metadata readFrom(File directory) {
        try (Storage storage = new RafStorage(new File(directory, LogFileUtils.METADATA_FILE), METADATA_SIZE, Mode.READ_WRITE)) {
            ...
            return new Metadata(segmentSize, segmentBitShift, maxSegmentsPerLevel, mmap, flushAfterWrite, asyncFlush);
        } catch (IOException e) {
